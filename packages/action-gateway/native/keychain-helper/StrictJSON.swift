...
enum KeychainStrictJSONFailure: Error {
  case rejected
}

func parseKeychainStrictJSON(
  _ data: Data,
  maximumBytes: Int,
  maximumDepth: Int = 16,
  ...
  guard !data.isEmpty,
    ...
    throw KeychainStrictJSONFailure.rejected
  }
  var scanner = KeychainStrictJSONScanner(
    data,
...
private struct KeychainStrictJSONScanner {
  ...
  mutating func parseRoot() throws {
    ...
    skipWhitespace()
    guard index == bytes.count else { throw KeychainStrictJSONFailure.rejected }
  }
  ...
  private mutating func parseString() throws -> String {
    ...
    while index < bytes.count {
      ...
      guard byte >= 0x20 else { throw KeychainStrictJSONFailure.rejected }
      if byte == UInt8(ascii: "\\") {
        guard index < bytes.count else { throw KeychainStrictJSONFailure.rejected }
        let escaped = bytes[index]
        index += 1
        if escaped == UInt8(ascii: "u") {
          guard index + 4 <= bytes.count else { throw KeychainStrictJSONFailure.rejected }
  ...
  private mutating func parseNumber() throws {
    _ = take(UInt8(ascii: "-"))
    guard let first = peek() else { throw KeychainStrictJSONFailure.rejected }
    if first == UInt8(ascii: "0") {
      ...
      while let next = peek(), (UInt8(ascii: "0")...UInt8(ascii: "9")).contains(next) {
        index += 1
      }
    }
    if take(UInt8(ascii: ".")) {
      guard let digit = peek(), (UInt8(ascii: "0")...UInt8(ascii: "9")).contains(digit)
      else {
        throw KeychainStrictJSONFailure.rejected
      }
      while let next = peek(), (UInt8(ascii: "0")...UInt8(ascii: "9")).contains(next) {
  ...
  private mutating func consume(_ expected: UInt8) throws {
    guard take(expected) else { throw KeychainStrictJSONFailure.rejected }
  }
