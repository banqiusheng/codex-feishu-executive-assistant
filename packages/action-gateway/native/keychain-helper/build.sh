...
readonly output_path="${1:-${repository_root}/dist/private-bin/assistant-keychain-helper}"
...
case "${build_mode}" in
  "")
    source_args=(
      ...
      "${script_dir}/VerifiedInstallManifest.swift"
      "${script_dir}/ParentVerifier.swift"
      "${script_dir}/StrictJSON.swift"
    )
    ;;
  --testing)
    (( $# == 2 )) || exit 2
    [[ "${output_name}" == "assistant-keychain-helper-testing" ]] || exit 2
  ...
  --kernel-probe)
    (( $# == 2 )) || exit 2
    [[ "${output_name}" == "assistant-keychain-kernel-probe" ]] || exit 2
    compiler_args=( -D ASSISTANT_TESTING -parse-as-library )
    source_args=(
      "${script_dir}/KernelProbe.swift"
      "${script_dir}/VerifiedInstallManifest.swift"
      "${script_dir}/ParentVerifier.swift"
      "${script_dir}/StrictJSON.swift"
    )
