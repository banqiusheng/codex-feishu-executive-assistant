...
readonly output_path="${1:-${repository_root}/dist/private-bin/assistant-gateway-control}"
...
typeset -a testing_args=()
typeset -a compiler_mode_args=()
typeset -a source_args=( "${script_dir}/main.swift" "${script_dir}/ParentVerifier.swift" "${script_dir}/StrictJSON.swift" )
...
case "${build_mode}" in
  "") ;;
  --testing)
    testing_args=( -D ASSISTANT_TESTING )
    ;;
  --kernel-probe)
    testing_args=( -D ASSISTANT_TESTING )
    compiler_mode_args=( -parse-as-library )
    source_args=( "${script_dir}/KernelProbe.swift" "${script_dir}/ParentVerifier.swift" "${script_dir}/StrictJSON.swift" )
    ;;
  *) exit 2 ;;
esac
