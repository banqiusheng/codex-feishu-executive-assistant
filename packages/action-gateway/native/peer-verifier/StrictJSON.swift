...
enum PeerStrictJSONFailure: Error {
  case rejected
}

func parsePeerStrictJSON(
  _ data: Data,
  maximumBytes: Int,
  maximumDepth: Int = 64,
  ...
  guard !data.isEmpty,
    ...
    throw PeerStrictJSONFailure.rejected
  }
  var scanner = PeerStrictJSONScanner(data, maximumDepth: maximumDepth, maximumNodes: maximumNodes)
  try scanner.parseRoot()
  return try JSONSerialization.jsonObject(with: data, options: [])
}

private struct PeerStrictJSONScanner {
  ...
  mutating func parseRoot() throws {
    ...
    skipWhitespace()
    guard index == bytes.count else { throw PeerStrictJSONFailure.rejected }
  }
  ...
  private mutating func parseObject(depth: Int) throws {
    ...
    while true {
      ...
      guard keys.insert(key).inserted else { throw PeerStrictJSONFailure.rejected }
      skipWhitespace()
      try consume(UInt8(ascii: ":"))
  ...
  private mutating func parseString() throws -> String {
    ...
    while index < bytes.count {
      ...
      guard byte >= 0x20 else { throw PeerStrictJSONFailure.rejected }
      if byte == UInt8(ascii: "\\") {
        guard index < bytes.count else { throw PeerStrictJSONFailure.rejected }
        let escaped = bytes[index]
        index += 1
        if escaped == UInt8(ascii: "u") {
          guard index + 4 <= bytes.count else { throw PeerStrictJSONFailure.rejected }
  ...
  private mutating func parseNumber() throws {
    _ = take(UInt8(ascii: "-"))
    guard let first = peek() else { throw PeerStrictJSONFailure.rejected }
  ...
  private mutating func consume(_ expected: UInt8) throws {
    guard take(expected) else { throw PeerStrictJSONFailure.rejected }
  }
