...
ACK/DNS 安全恢复与授权页自动打开的[补修设计](docs/superpowers/specs/2026-07-25-feishu-ack-recovery-and-zero-copy-auth-design.md)及其[实现计划](docs/superpowers/plans/2026-07-25-ack-recovery-and-zero-copy-auth.md)已经完成规格确认。持久数据库 ACK 门禁、单次新消息的“发送成功 → 私有 marker → 数据库 `ACKNOWLEDGED` → worker wake”接线，以及启动前 marker/账本对账已经实现。仅 `ENOTFOUND` / `EAI_AGAIN` 的运行时重试协调器、doctor、OAuth、marker v2/taskId 绑定和真实 E2E 仍待完成。
